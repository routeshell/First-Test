...
To realize this requirement, a class named Job Offer will handle the user interface for entering job offer details, utilizing the attributes inherited from the parent Job class. The application flow will manage transitions back to the main menu handled by GUI class, for entering another offer, or to the comparison function based on the user's choice. As it has been shown in the UML diagram a Job Offer class can be zero to as many, while the Current Job class would be 0 or 1 and they have a child-parent relation with Job class and an association with Job Comparison.
...
To realize this requirement, a class named Comparison Settings Weights Adjustment will handle the comparison settings, utilizing the described attributes in requirement (4) with the datatype of int, default value of 1 and value between 0 to 9. It includes an operation called “setWeight” substituting the inserted weight with the default values and operations “save”, “cancel”, each for the corresponding task described in the requirement. Return to the main menu will be handled within the GUI implementation.
...
To realize this requirement, I added a float “yearlySalaryAdjustedForCostOfLiving” and a float “yearlyBonusAdjustedForCostOfLiving” to the ‘Job’ class to represent the Yearly Salary Adjusted for cost of living (AYS) and Yearly Bonus Adjusted for cost of living (AYB). The “jobScoreCalculation” operation was added to the “Job” class does the calculation based on the two values of the AYS and AYB, the other values from either current job and/or job offer attribute and the weights derived from “Comparison Settings Weights Adjustment”.
